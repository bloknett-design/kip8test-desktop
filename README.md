...
- **Windows (универсальный)** — `KIPiA-Test-Setup-2.1.8.exe` — сам определяет разрядность (64/32-бит)
- **Windows 64-бит** — `KIPiA-Test-Setup-2.1.8-x64.exe` (64-бит, меньше размер)
...
Windows-сборки двух разрядностей (x64 и ia32/Win32) + универсальный установщик;
каждая разрядность выбирает свой файл из общего `latest.yml`.
