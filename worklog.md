...
## Task 347 — догон справочника Code.gs (синхрон с kip8test, 2026-09-08)

Заявка: «проверить, чтобы в репозиториях проектов все файлы были
актуальными» (сопровождение Tasks 346/347).

- scripts/Code.gs — догон с версии era Task 118 до байт-в-байт ≡
  kip8test (справочник отставал на +211 строк): Task 284 URL
  развёртывания, Task 295 GATE_ACTIONS (шлюз RoleMatrixGate), Task 346
  роутер `Auth.verifyOTP(…, payload)`, Task 347 вызов
  `Utils.cleanupStaleSessions()` в hourlyCleanup;
- Тесты: **218/0** — без изменений (guard изоляции Task 345 жив);
